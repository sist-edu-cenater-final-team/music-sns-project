...
function getmusicId() {
	...
	apiRequest(() =>
		...
		const profile = $("div#profileMusic");
		let v_html = '';
		let musicIdArr = json.map(item => item.musicId);

		if (musicIdArr.length === 0) {
			v_html += `<div class="no-music">등록된 음악이 없습니다</div>`;
			profile.html(v_html);
			return;
		}
		else {
			getMusicList(musicIdArr);
		}
...
function getMusicList(musicIdArr) {
	// 한 번에 musicList 가져오기
	apiRequest(() =>
		new Promise((resolve, reject) => {
			$.ajax({
